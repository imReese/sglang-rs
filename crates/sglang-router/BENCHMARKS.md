# sglang-router microbench harness + SMG comparison

This file pairs with `crates/sglang-router/benches/` and the SMG
...
sglang-router:
```bash
cargo bench -p sglang-router --bench tree_lookup     -- --sample-size 30 --measurement-time 3
cargo bench -p sglang-router --bench policy_select   -- --sample-size 30 --measurement-time 3
```
...
that the sglang-router routing primitives are in the same ballpark as the
...
| Bench | sglang-router | SMG TokenTree | Notes |
|---|---|---|---|
| Insert 64 blocks for 1 worker (medium case) | `hashtree_insert/128` ≈ 21.5 µs | `token_insert_10w_4096tok` ≈ 1.05 µs | Numbers not directly comparable — SMG counts per-token insert, sglang-router counts per-block insert. SMG inserts 4096 tokens at a fixed `block_size`; sglang-router inserts 128 pre-hashed `i64` block-hashes. The hashing step (`compute_block_hashes`) is upstream of `HashTree` and not measured here. |
| Match request prefix | `hashtree_match_prefix/w64_bpw128_q64` ≈ 47 ns | `token_match_10w_4096tok` ≈ 1.24 µs | sglang-router's match is a short-circuit walk over `i64` hashes; SMG's match tokenizes + hashes per-call. The fair comparison includes `compute_block_hashes` cost (~ tens of µs depending on prompt length). |
...
upstream dominates in real traffic. The number proves that sglang-router's
...
| `random`          | 16 ns | 36 ns | 137 ns | 471 ns | SMG random is also O(1) per `rand::random()` call; sglang-router's variant grows with n because it `Vec::iter().nth(idx)`. **Action item:** drop sglang-router to O(1) by indexing the slice directly. |
